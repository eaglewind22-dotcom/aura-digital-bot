...
async function checkForcedJoinAndShop(ctx, next) {
    ...
    try {
        ...
        if (['left', 'kicked'].includes(member.status)) {
            // FIX: Channel Link အမှန်သို့ ပြောင်းလဲထားပါသည်
    ...
    handleMenuInterruption(ctx, 'GUIDE', () => {
        try {
            ctx.replyWithMarkdown(`💡 *Aura Digital - လမ်းညွှန်ချက်များ*\n\n` +
                                  `၁။ *စိန်ဖြည့်နည်း:* Store ထဲဝင်ပြီး ပစ္စည်းရွေး၊ MLBB ID ရိုက်ထည့်ကာ Ngweလွှဲပြေစာ ပို့ပေးရုံဖြင့် ရပါပြီ။\n` +
...
bot.action('int_continue', async (ctx) => {
    try {
        ...
        if (session.step === 'AWAITING_RECEIPT') ctx.reply('👉 ဆက်လက်ပြီး Ngweလွှဲပြေစာ (Receipt Screenshot) ကို ပို့ပေးပါဦးဗျာ။');
...
bot.action('claim_coupon', async (ctx) => {
    try {
        ...
        ctx.reply('🎟️ Point ၅,၀၀၀ အား ကူပွန် ၁ ရွက်အဖြစ် အောင်မြင်စွာ လလဲလှယ်ပြီးပါပြီဗျာ။ ဝယ်ယူချိန်တွင် သုံးနိုင်ပါပြီ။');
    } catch(e) {}
});
...
bot.hears('💰 လက်ရှိဈေးနှုန်းများကြည့်ရန်', async (ctx) => {
    ...
    // FIX: Syntax Error ပြင်ဆင်ပြီးပါပြီ
    ctx.reply(msg, Markup.inlineKeyboard(kb));
});

bot.hears('⚙️ အချက်အလက်များ ပြင်ရန်', (ctx) => {
    if (!isAdmin(ctx)) return;
    ctx.reply('⚙️ ပြင်ဆင်လိုသည့် Ngweပေးချေမှုစနစ် အချက်အလက်ကို ရွေးချယ်ပါဗျာ -', Markup.inlineKeyboard([
    ...
    try {
        ...
        if (type === 'receipt') {
            ...
            bot.telegram.sendMessage(o.telegramId, `⚠️ လူကြီးမင်း၏ အော်ဒါ #${oid} အတွက် ပေးပို့လာသော ပြေစာပုံမှာ မှားယွင်းနေပါသည်ဗျာ။ ကျေးဇူးပြု၍ ပြေစာပုံအမှန်ကို အောက်ကခလုတ်နှိပ်ပြီး ပြန်လည်ပေးပို့ပေးပါဗျာ။`, {
                ...Markup.inlineKeyboard([[Markup.button.callback('📷 ပြေစာပုံအမှန် ပြန်ပို့မည်', `send_rec_${oid}`)], [Markup.button.callback('❌ ဤअော်ဒါအား လုံးဝဖျက်သိမ်းမည်', `user_cancel_order_${oid}`)]])
            }).catch(()=>{});
        } else if (type === 'id') {
...
bot.action(/^send_(rec|id|bal)_(.+)$/, async (ctx) => {
    try {
        ...
        if (type === 'rec') {
            ...
            ctx.reply('📷 ကျေးဇူးပြု၍ Ngweလွှဲပြေစာပုံအမှန်အား စနစ်သို့ တိုက်ရိုက် ပို့ပေးပါဗျာ။');
        } else if (type === 'id') {
...
// [3] Polling Mode နဲ့ အလုပ်လုပ်မည့် အပိုင်း (ဒီအပိုင်းက အဓိကပါ)
bot.launch({ dropPendingUpdates: true })
    .then(() => console.log('✅ Bot started in Polling Mode!'));

// [4] Render Port စစ်ဆေးမှု ကျော်ဖြတ်ရန်
